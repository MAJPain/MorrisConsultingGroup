...
import {
  ...
  ebaSections,
  type EbaInline,
} from "@/content/executive-and-board-advisory";

export const metadata: Metadata = {
  title: "Executive & Board Advisory",
  description:
    "A core practice serving the decision makers responsible for consequential outcomes at the executive and board level — CEO advisory, board advisory, effectiveness reviews, enterprise diagnostics, and executive succession.",
};

function InlineNodes({ nodes }: { nodes: EbaInline[] }) {
  return (
    <>
      {nodes.map((node, i) => {
...
export default function ExecutiveAndBoardAdvisoryPage() {
  return (
    <>
      <PageHeader
        eyebrow={ebaHeader.eyebrow}
        title={ebaHeader.title}
        subtitle={ebaHeader.subtitle}
        lede={ebaHeader.lede}
      />

      {ebaSections.map((section) => (
